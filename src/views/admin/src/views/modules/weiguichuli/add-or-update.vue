<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'" label="借阅号" prop="jieyuehao">
            <el-input v-model="ruleForm.jieyuehao" 
                placeholder="借阅号" readonly></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.jieyuehao" label="借阅号" prop="jieyuehao">
              <el-input v-model="ruleForm.jieyuehao" 
                placeholder="借阅号" readonly></el-input>
          </el-form-item>
        </div>
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="处理类型" prop="chulileixing">
          <el-select :disabled="ro.chulileixing" v-model="ruleForm.chulileixing" placeholder="请选择处理类型">
            <el-option
                v-for="(item,index) in chulileixingOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="处理类型" prop="chulileixing">
	      <el-input v-model="ruleForm.chulileixing"
                placeholder="处理类型" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="缴纳罚金" prop="jiaonafajin">
          <el-input v-model="ruleForm.jiaonafajin" 
              placeholder="缴纳罚金" clearable  :readonly="ro.jiaonafajin"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="缴纳罚金" prop="jiaonafajin">
              <el-input v-model="ruleForm.jiaonafajin" 
                placeholder="缴纳罚金" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
  ...
  data() {
	...
	ispay : false,
      ...
      rules: {
          ...
          jiaonafajin: [
                { required: true, message: '缴纳罚金不能为空', trigger: 'blur' },
                { validator: validateIntNumber, trigger: 'blur' },
          ],
          dengjishijian: [
          ],
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.chulileixing = true;
          ...
          if(o=='jiaonafajin'){
            this.ruleForm.jiaonafajin = obj[o];
	    this.ro.jiaonafajin = true;
            continue;
          }
          if(o=='dengjishijian'){
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.chulileixingOptions = "逾期处理,丢失图书".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `weiguichuli/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `weiguichuli/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.weiguichuliCrossAddOrUpdateFlag = false;
						 this.parent.search();
